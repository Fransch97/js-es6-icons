...
const conditionsChecker = (arrary,value)=> {arrary.forEach(array =>{console.log(array.forEach(element=>{(element.type === value)? resetPrint(array): ""}))})}; conditionsChecker(typesObjArr,selectTag.value);
//get row
const row = document.querySelector('.bana');
//color number # length
const numberLength = 6;
//event "change"
selectTag.addEventListener('change',()=>setInterval(()=> {(selectTag.value === "all")? resetPrint(data):conditionsChecker(typesObjArr,selectTag.value)},100));//resetPrint(data)
